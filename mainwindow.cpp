...
void MainWindow::on_pushButton_12_clicked()
{
    QSqlQuery query,qry,q;
        ...
        qry.prepare("Select * from personnel where type ='admin'");
        if(qry.exec())
        {
            while (qry.next())
            {
        ...
        q.prepare("Select * from personnel where type='utilisateur'");
        if(q.exec())
        {
            while (q.next())
            {
    ...
    QPieSeries *series = new QPieSeries();
                series->append("admin",l1);
                series->append("utilisateur",l2);
